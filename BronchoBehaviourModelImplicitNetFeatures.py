...
class BronchosopyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.cache.keys():
            ...
            self.cache[idx] = (features, stateInput, goalInput, action)
            
            return features, stateInput, goalInput, action
...
class DataAugmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features, state, goal, label = self.primaryDataset[actualIdx]
...
class MultiInputModel(nn.Module):
    def __init__(self, numStates=3, numGoal=4, numFeatures=1260, classes=6):
        super(MultiInputModel, self).__init__()
        


        
        self.fc1_features = nn.Linear(in_features=numFeatures, out_features=256)
        ...
        concatenated_size = 32 + 32 + 256
        
        # Fully connected layers for the combined output
        #self.fc1_combined = nn.Linear(in_features=concatenated_size, out_features=128)
    ...
    def forward(self, features, state, goal):
        # CNN pathway for image input
        
        #print(f"Features: {features.shape}")
        features = F.relu(self.fc1_features(features))

        state = F.relu(self.fc1_state(state))
        goal = F.relu(self.fc1_goal(goal))

        x = torch.cat((features, goal, state), dim=1)
...
def main(epochs = 50, learningRate = 0.0001, modelSavePath = "modelImplicit.pth"):
    ...
    transform = T.Compose([
        ...
        T.Resize((224, 224)),                # Resize images to 50x50 pixels
    ...
    output_folder = create_output_folder("runs/implictTrainingPreFeatures")
    ...
    sampleFeatures, sampleState, sampleGoal, sampleLabel = dataSample

    numFeatures = len(sampleFeatures)
    numStates = len(sampleState)
    numGoal = len(sampleGoal)
    numClasses = len(sampleLabel)
